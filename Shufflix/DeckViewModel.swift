...
final class DeckViewModel: ObservableObject {
  ...
  @Published var errorMessage: String?        // <— settable so extensions can write to it
  ...
  @Published private(set) var searchResults: [TitleItem] = []
  ...
  // Persistence (internal so extensions can access)
  ...
  func swipe(_ item: TitleItem, liked didLike: Bool) {
    ...
    // Non-blocking remote persist (extension lives in DeckViewModel+Supabase)
    Task { [tmdbID = Int64(item.id), media = item.mediaType.rawValue] in
      if didLike { self.like(tmdbID: tmdbID, media: media) }
      else { self.skip(tmdbID: tmdbID, media: media) }
    }
  }
  ...
  private var allowedFeeds: [Feed] {
    ...
    case .movie:
      // movies + trending
      return [.trendingWeek, .trendingDay, .popularMovie, .topMovie, .discoverMovie]
    case .tv:
      // tv + trending
      return [.trendingWeek, .trendingDay, .popularTV, .topTV, .discoverTV]
    }
  }
  ...
  private func handleIncomingBatch(_ additions: [TitleItem]) -> Int {
    ...
    let newSource = (source + additions).unique(by: \.id)
    if newSource.count != source.count {
      source = newSource
  ...
  private func injectSimilar(for item: TitleItem) async {
    do {
      ...
      // Update `source` once
      let newSource = (source + top).unique(by: \.id)
      if newSource.count != source.count {
        source = newSource
        rebuildAvailableFilters(from: source)
      }
  ...
  private func rebuildAvailableFilters(from items: [TitleItem]) {
    ...
    let newProviders = Array(providers).sorted { $0.localizedCaseInsensitiveCompare($1) == .orderedAscending }
    let newGenres    = Array(genres).sorted { $0.localizedCaseInsensitiveCompare($1) == .orderedAscending }

    // Only publish changes if they differ (less UI churn)
    if newProviders != availableProviders { availableProviders = newProviders }
    if newGenres != availableGenres { availableGenres = newGenres }
  }
...
// MARK: - Search (debounced, fuzzy, forgiving)
...
private extension DeckViewModel {
  ...
  func performSearch() async {
    ...
    searchTask = Task { [q] in
      do {
        // Fast, recall-first TMDB search. It already normalizes and de-dupes.
        let raw = try await TMDBService.searchTitles(
          query: q,
          type: .all,
          pageLimit: 3,
          region: Constants.TMDB.defaultRegion
        )
        // Rank again locally to incorporate our looser rules for filter/search bars.
